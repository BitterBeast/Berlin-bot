...
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, get_time
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            h = await client.send_message(group_id, reply_text, disable_web_page_preview=True)
                            await asyncio.sleep(600)
                            await h.delete()
                        else:
                            button = eval(btn)
